  ...
  return (
    <div className="safe-bottom">
      ...
      <div style={{ display: 'flex', borderBottom: '0.5px solid var(--border)', padding: '0 16px' }}>
        {([['browse', 'Все слова'], ['review', 'Повторение'], ['favorites', 'Избранное']] as [Tab, string][]).map(([t, label]) => (
        ...
        <div style={{ padding: '16px' }}>
          {reviewWords.length === 0 ? (
            <div style={{ textAlign: 'center', paddingTop: 60 }}>
              ...
              <div style={{ color: 'var(--text-2)', fontSize: 14, marginTop: 8 }}>Следующее повторение завтра</div>
            </div>
          ) : (
            <>
              <div style={{ fontSize: 13, color: 'var(--text-2)', marginBottom: 16, textAlign: 'center' }}>
                {reviewIndex + 1} из {reviewWords.length}
              </div>
              <div style={{
                background: 'var(--bg)', border: '0.5px solid var(--border)',
                borderRadius: 'var(--radius-card)', padding: 28, textAlign: 'center', marginBottom: 16,
                minHeight: 200, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 12,
              }}>
                <div style={{ fontSize: 32, fontWeight: 500 }}>{reviewWords[reviewIndex]?.bashkir}</div>
                ...
                {!showTranslation ? (
                  <button onClick={() => setShowTranslation(true)} className="btn-outline" style={{ marginTop: 8 }}>
                    Показать перевод
                  </button>
                ) : (
                  <div style={{ animation: 'fadeIn 0.25s ease' }}>
                    <div style={{ fontSize: 20, color: 'var(--text-2)', marginTop: 8 }}>{reviewWords[reviewIndex]?.russian}</div>
                  ...
                  {([1, 2, 3, 4] as const).map(rating => {
                    ...
                    return (
                      <button key={rating} onClick={() => handleReviewRate(rating)}
                        style={{ background: 'var(--surface)', border: `0.5px solid ${meta.color}`, borderRadius: 12, padding: '12px 8px', cursor: 'pointer' }}>
      ...
      {tab === 'favorites' && (
        favorites.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '60px 32px' }}>
            ...
            <div style={{ fontSize: 16, fontWeight: 500 }}>Пока нет избранных</div>
            <div style={{ color: 'var(--text-2)', fontSize: 13, marginTop: 8 }}>Нажми ❤️ на любом слове</div>
          </div>
        ) : favorites.map(word => (
          <WordCard key={word.id} word={word}
            uv={userVocab.get(word.id)}
...
function WordCard({ word, uv, onFavorite, onAdd }: {
  word: VocabWord
  uv?: UserVocab
  onFavorite: () => void
  onAdd: () => void
