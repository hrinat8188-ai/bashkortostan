...
const ALPHABET = [
  ...
  { letter: 'Ыы', sound: '[ɯ]', like: 'задний ряд, башкирское ы', special: true },
  ...
  return (
    <div className="safe-bottom">
      ...
      <div style={{ display: 'flex', gap: 8, padding: '12px 16px 0' }}>
        {[['all', 'Все буквы'], ['special', 'Особые (9 букв)']].map(([key, label]) => (
          <button key={key} onClick={() => setFilter(key as 'all' | 'special')}
            className={filter === key ? 'btn-primary' : 'btn-outline'}
      ...
      {selected && (
        <div style={{
          ...
          {selected.special && (
            <div style={{ background: 'var(--gold-light)', borderRadius: 10, padding: '8px 12px', fontSize: 13, color: '#7a4f0a' }}>
